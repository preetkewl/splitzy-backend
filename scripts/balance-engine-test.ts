...

{
  ...
  check(
    ...
    shares.every((s) => s.sharePaise === 25_000),
    shares,
  );
  check(
    'clean division: SUM(shares) === amount',
    shares.reduce((s, x) => s + x.sharePaise, 0) === 100_000,
  );
}

{
  ...
  const sum = shares.reduce((s, x) => s + x.sharePaise, 0);
  check('remainder: SUM(shares) === amount', sum === 100, shares);
  check(
    'remainder: payer absorbs remainder',
    shares.find((x) => x.userId === 'b')?.sharePaise === 34,
    shares,
  );
  check(
    'remainder: non-payers get floor',
    shares.find((x) => x.userId === 'a')?.sharePaise === 33 &&
      shares.find((x) => x.userId === 'c')?.sharePaise === 33,
  );
}

{
  ...
  check('single participant: payer takes everything', shares.length === 1 && shares[0]?.sharePaise === 123);
}

{
  ...
  const amount = 100_000_000_000; // ₹100 crore in paise — at the bound
  ...
  check(
    ...
    shares.find((x) => x.userId === 'u3')?.sharePaise === amount - Math.floor(amount / 7) * 6,
  );
}
...

{
  ...
  const sum = net.reduce((s, b) => s + b.netPaise, 0);
  ...
  check(
    ...
    net.find((b) => b.userId === 'a')?.netPaise === 17_000,
    net,
  );
  check(
    'b: paid  9k − owes 13k = −4k',
    net.find((b) => b.userId === 'b')?.netPaise === -4_000,
    net,
  );
  check(
    'c: paid  0  − owes 13k = −13k',
    net.find((b) => b.userId === 'c')?.netPaise === -13_000,
    net,
  );
}

{
  // Remainder fixture: every expense leaves a 1-paise remainder absorbed
  // by the payer. The frontend's algorithm would drift; ours stays at 0.
  const memberIds = ['a', 'b', 'c'];
  const expenses: ExpenseInput[] = [
    { payerId: 'a', amountPaise: 100, participants: BalanceEngine.splitEqual(100, memberIds, 'a') },
    { payerId: 'b', amountPaise: 100, participants: BalanceEngine.splitEqual(100, memberIds, 'b') },
    { payerId: 'c', amountPaise: 100, participants: BalanceEngine.splitEqual(100, memberIds, 'c') },
  ];
  const net = BalanceEngine.computeNetBalances(memberIds, expenses);
  const sum = net.reduce((s, b) => s + b.netPaise, 0);
...

{
  ...
  check(
    ...
    net.reduce((s, b) => s + b.netPaise, 0) === 0,
    net,
  );
  check(
...

{
  ...
  check('baseline: a +20k', noSettlements.find((x) => x.userId === 'a')?.netPaise === 20_000);
  check('baseline: b -10k', noSettlements.find((x) => x.userId === 'b')?.netPaise === -10_000);
  check('baseline: c -10k', noSettlements.find((x) => x.userId === 'c')?.netPaise === -10_000);
  ...
  const partialSettled = BalanceEngine.computeNetBalances(memberIds, expenses, [
    { fromUserId: 'b', toUserId: 'a', amountPaise: 10_000 },
  ]);
  check(
    'partial settlement: b → a 10k zeroes b',
    partialSettled.find((x) => x.userId === 'b')?.netPaise === 0,
    partialSettled,
  );
  check(
    'partial settlement: a drops to +10k',
    partialSettled.find((x) => x.userId === 'a')?.netPaise === 10_000,
  );
  check('partial settlement: c unchanged at -10k', partialSettled.find((x) => x.userId === 'c')?.netPaise === -10_000);
  check(
    'partial settlement: SUM(net) still 0',
    partialSettled.reduce((s, x) => s + x.netPaise, 0) === 0,
  );

  // Both b → a 10k AND c → a 10k → everyone settled at zero.
  const allSettled = BalanceEngine.computeNetBalances(memberIds, expenses, [
    { fromUserId: 'b', toUserId: 'a', amountPaise: 10_000 },
    { fromUserId: 'c', toUserId: 'a', amountPaise: 10_000 },
  ]);
  check(
    'fully settled: every net is 0',
    allSettled.every((x) => x.netPaise === 0),
    allSettled,
  );
  ...
  const overpaid = BalanceEngine.computeNetBalances(memberIds, expenses, [
    { fromUserId: 'b', toUserId: 'a', amountPaise: 15_000 },
  ]);
  check(
    'over-payment: SUM(net) still 0 (engine never clamps)',
    overpaid.reduce((s, x) => s + x.netPaise, 0) === 0,
    overpaid,
  );
  check('over-payment: a drops to +5k', overpaid.find((x) => x.userId === 'a')?.netPaise === 5_000);
  check('over-payment: b flips to +5k', overpaid.find((x) => x.userId === 'b')?.netPaise === 5_000);
}

console.log('\n· computeNetBalances rejects malformed settlements');
expectThrow('settlement with zero amount', () =>
  BalanceEngine.computeNetBalances(['a', 'b'], [], [
    { fromUserId: 'a', toUserId: 'b', amountPaise: 0 },
  ]),
);
expectThrow('settlement with negative amount', () =>
  BalanceEngine.computeNetBalances(['a', 'b'], [], [
    { fromUserId: 'a', toUserId: 'b', amountPaise: -1 },
  ]),
);
expectThrow('settlement from = to', () =>
  BalanceEngine.computeNetBalances(['a', 'b'], [], [
    { fromUserId: 'a', toUserId: 'a', amountPaise: 100 },
  ]),
);
...

{
  ...
  const balances: NetBalance[] = [
    { userId: 'aarya', netPaise: -231_000 },
    { userId: 'aarav', netPaise: 849_000 },
    { userId: 'meera', netPaise: -171_000 },
    { userId: 'kabir', netPaise: -447_000 },
  ];
  const t = BalanceEngine.simplify(balances);
  check('Goa: 3 transfers', t.length === 3, t);
  check(
    'Goa: SUM(transfers) === total debt',
    t.reduce((s, x) => s + x.amountPaise, 0) === 849_000,
  );
  ...
  check('Goa: stable order largest debtor first', t[0]?.fromUserId === 'kabir' && t[0]?.amountPaise === 447_000);
  check('Goa: second is aarya 231k', t[1]?.fromUserId === 'aarya' && t[1]?.amountPaise === 231_000);
  check('Goa: third is meera 171k', t[2]?.fromUserId === 'meera' && t[2]?.amountPaise === 171_000);
}

{
  // Determinism — run simplify with permuted input order, expect same output.
  const a: NetBalance[] = [
    { userId: 'u1', netPaise: 100 },
    { userId: 'u2', netPaise: -50 },
    { userId: 'u3', netPaise: -50 },
  ];
  const b: NetBalance[] = [
    { userId: 'u3', netPaise: -50 },
    { userId: 'u1', netPaise: 100 },
...

{
  ...
  const t = BalanceEngine.simplify([
    { userId: 'a', netPaise: 1234 },
    { userId: 'b', netPaise: -1234 },
  ]);
  check('single pair: 1 transfer', t.length === 1);
  check(
    'single pair: b → a, 1234',
    t[0]?.fromUserId === 'b' && t[0]?.toUserId === 'a' && t[0]?.amountPaise === 1234,
  );
}

{
  ...
  const balances: NetBalance[] = [
    { userId: 'u1', netPaise: 1000 },
    { userId: 'u2', netPaise: 500 },
    { userId: 'u3', netPaise: 200 },
    { userId: 'u4', netPaise: -700 },
    { userId: 'u5', netPaise: -500 },
    { userId: 'u6', netPaise: -300 },
    { userId: 'u7', netPaise: -200 },
  ];
  const t = BalanceEngine.simplify(balances);
  const sum = t.reduce((s, x) => s + x.amountPaise, 0);
  ...
  check('mixed: every transfer is positive', t.every((x) => x.amountPaise > 0));
}
