 ...
 * Percentage splits almost always produce non-integer minor unit values. For
 * example: 33.33% of ₹100 = 33.33 minor units, which is not a whole minor unit.
 * We must distribute amountMinor as an exact integer sum. LRM does this:
 *
 *   Step 1 — Floor pass.
 *     For each participant i, compute the "ideal" share:
 *       ideal_i = basisPoints_i × amountMinor / 10 000
 *     Take the floor:
 *       floor_i = ⌊ideal_i⌋
 *     Compute the fractional remainder (as an integer numerator, not a float):
 *       rem_i = (basisPoints_i × amountMinor) mod 10 000
 *
 *   Step 2 — Distribute surplus.
 *     remainder = amountMinor − Σ(floor_i)
 ...
 *     Award 1 extra minor unit to the first `remainder` participants.
 *
 *   Step 3 — Result.
 *     shareMinor_i = floor_i + (1 if participant received extra, else 0)
 *     SUM(shareMinor) = Σ(floor_i) + remainder = amountMinor ✓
 *
 * ── Why LRM is preferred over "payer absorbs remainder" ──────────────────────
 * For EQUAL splits, payer-absorbs is fine because there is only ever 1 minor units
 * of remainder and the payer is already the most accountable party.
 * For percentage splits, the remainder can be up to n−1 minor units. LRM distributes
 ...
 * receive identical shareMinor when possible (tie-break by userId is stable).
 *
 * ── Why BigInt, not float64 ───────────────────────────────────────────────────
 * basisPoints ≤ 10 000; amountMinor ≤ 100 000 000 000 (100 crore).
 ...
 * always receives the same extra-minor-unit assignment regardless of the order
...
export class PercentSplitCalculator implements SplitCalculator {
  ...
  calculate(
    ...
    // ── Step 2: distribute the surplus minor unit ─────────────────────────────────
    ...
    // The first `extraMinor` participants in the sorted order each receive +1.
